...
export default function Reservation() {
  ...
  const inp: React.CSSProperties = { fontFamily: "'DM Sans', sans-serif", fontSize: '0.9rem', fontWeight: 300, color: restaurant.colors.text, background: `rgba(107,58,42,0.06)`, border: `1px solid rgba(107,58,42,0.12)`, padding: '0.85rem 1rem', outline: 'none', width: '100%', borderRadius: '8px', transition: 'border-color 0.2s' }
  ...
  return (
    <section id="reserve" style={{ background: restaurant.colors.primary, padding: isMobile ? '3rem 1.5rem' : '7rem 2rem' }}>
      ...
      <FadeIn direction="right" delay={0.1}>
        <div style={{ background: restaurant.colors.bg, padding: '3rem', borderRadius: '16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
            <div><label style={lbl}>Vārds</label><input name="firstName" value={form.firstName} onChange={onChange} placeholder="Jānis" style={inp} onFocus={e => e.target.style.borderColor = restaurant.colors.primary} onBlur={e => e.target.style.borderColor = 'rgba(107,58,42,0.12)'} /></div>
            <div><label style={lbl}>Uzvārds</label><input name="lastName" value={form.lastName} onChange={onChange} placeholder="Bērziņš" style={inp} onFocus={e => e.target.style.borderColor = restaurant.colors.primary} onBlur={e => e.target.style.borderColor = 'rgba(107,58,42,0.12)'} /></div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
            <div><label style={lbl}>Datums</label><input type="date" name="date" value={form.date} onChange={onChange} style={inp} onFocus={e => e.target.style.borderColor = restaurant.colors.primary} onBlur={e => e.target.style.borderColor = 'rgba(107,58,42,0.12)'} /></div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
            ...
            <div><label style={lbl}>Telefons</label><input type="tel" name="phone" value={form.phone} onChange={onChange} placeholder="+371 ..." style={inp} onFocus={e => e.target.style.borderColor = restaurant.colors.primary} onBlur={e => e.target.style.borderColor = 'rgba(107,58,42,0.12)'} /></div>
          </div>
          <div style={{ marginBottom: '1.5rem' }}><label style={lbl}>Īpašie lūgumi</label>
            <textarea name="notes" value={form.notes} onChange={onChange} placeholder="Diētiskie ierobežojumi, svētki, alerģijas..." rows={3} style={{ ...inp, resize: 'vertical' }} onFocus={e => e.target.style.borderColor = restaurant.colors.primary} onBlur={e => e.target.style.borderColor = 'rgba(107,58,42,0.12)'} />
          </div>

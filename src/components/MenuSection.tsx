        ...
        <FadeIn delay={0.1}>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: '1.5rem', marginBottom: '5rem' }}>
            {restaurant.menu.Mains.map((item, i) => (
              ...
              >
                <div style={{ height: '320px', overflow: 'hidden', background: restaurant.colors.bgAlt }}>
                  <img src={item.img} alt={item.name} style={{ width: '100%', height: '100%', objectFit: 'contain', transition: 'transform 0.5s' }}
                    onMouseEnter={e => (e.target as HTMLImageElement).style.transform = 'scale(1.06)'}
                    onMouseLeave={e => (e.target as HTMLImageElement).style.transform = 'scale(1)'}
                    onError={e => (e.target as HTMLImageElement).style.display = 'none'} />
                </div>
                <div style={{ padding: '1.4rem 1.5rem' }}>
                  <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.78rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: restaurant.colors.text, marginBottom: '0.3rem' }}>{item.name}</p>
                  <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.82rem', fontWeight: 300, color: restaurant.colors.gray, lineHeight: 1.6 }}>{item.desc}</p>
                </div>
              </div>
            ))}
          </div>
